...
from src.config.settings import transformer_name
...
def get_data():
    bad_file_path = 'datasets/train_data_bad.txt'
    good_file_path = 'datasets/train_data_good.txt'
...
def get_train_data():
    ...
    train_df, val_df = train_test_split(train_val_df, test_size=0.25)  # Adjust the test_size as per your requirement
...
def get_sentiment_analysis_model():
    ...
    model.load_state_dict(torch.load("ml_model/transformer.pt",map_location=torch.device('cpu')))
